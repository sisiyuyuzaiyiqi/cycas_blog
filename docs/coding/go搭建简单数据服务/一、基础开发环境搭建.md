...
2. go插件使用了一批Go工具来提供丰富的功能。这些工具会默认安装在GOPATH路径。如果你想安装到其他路径，在配置文件中配置`go.toolsGoPath`。
3. 界面右下角将会弹出`Analysis Tools Missing`对话框，点击它将安装所有依赖的Go工具。同样也可以使用命令安装：`Go: Install/Update tools`来安装/升级。
...
## 补充阅读（可跳过）：windows开发环境 - 使用 goland
