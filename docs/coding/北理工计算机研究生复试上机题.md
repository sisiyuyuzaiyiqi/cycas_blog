...
任何一个大于 2 的偶数均可表示为两个素数之和。输入 m, n(6<=m<=n<=50),则把[m, n]内的所有偶数表示成两个素数之和的形式。输出这些素数及其出线的次数，输出次序按照素数出现的次数从多到少输出；若出线次数相同，按照素数从大到小输出；若偶数有多种素数相加形式，则把所有的情况都输出，每种情况占一行。
...
3. 遍历[m,n]内的所有偶数，找出 vect 中所有相加等于该偶数的素数对（包括相等）
